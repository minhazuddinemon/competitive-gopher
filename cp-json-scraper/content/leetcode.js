  ...
  if (message.action === "START_SCRAPING") {
    scrapeLeetCode().then(payload => {
      if (payload) {
        ...
        sendResponse({ success: false, error: "Failed to scrape LeetCode problem page." });
      }
    });
  }
...
async function scrapeLeetCode() {
  try {
    const title = document.querySelector('span.text-title-large')?.innerText || "LeetCode Problem";

    // Extract test cases from text sample code boxes
    const preBlocks = document.querySelectorAll('div[data-track-load="description_content"] pre, pre');
    let tests = [];
    ...
    preBlocks.forEach((block) => {
      ...
      if (text.includes("Input:") && text.includes("Output:")) {
        ...
        const outputMatch = text.match(/Output:\s*([\s\S]*?)(?=Explanation:|Example|\n\n|$)/);
    ...
    for (let link of topicLinks) {
      ...
      if (href.includes('/tag/binary-tree') || tagText === 'binary tree') {
        ...
        break;
      } else if (href.includes('/tag/linked-list') || tagText === 'linked list') {
...
function getLeetCodeCode() {
  ...
  if (codeMirrorLines && codeMirrorLines.length > 0) {
    return Array.from(codeMirrorLines).map(l => l.innerText).join('\n');
  }
  return "";
}
